 ...
 */
abstract class AbstractFilterMapReduce<T, U> {
 filter(pred: (item: T) => boolean, list: T[]): T[] {
    const result: T[] = [];
 ...
 * @tparam number Tipo de los elementos de la lista de entrada y de la lista de salida.
 */
class FilterMapAddReduce extends AbstractFilterMapReduce<number, number> {
...
/**
 * @brief Clase concreta que implementa el algoritmo de filtrado, mapeo y reducción sobre una lista de números. En el método de reducción, se realiza la operación de resta.
 * @tparam number Tipo de los elementos de la lista de entrada y de la lista de salida.
 */
class FilterMapSubReduce extends AbstractFilterMapReduce<number, number> {
  filter(pred: (item: number) => boolean, list: number[]): number[] {
    const result: number[] = [];
    for (const item of list) {
      if (pred(item)) {
...
/**
 * @brief Clase concreta que implementa el algoritmo de filtrado, mapeo y reducción sobre una lista de números. En el método de reducción, se realiza la operación de multiplicación.
 * @tparam number Tipo de los elementos de la lista de entrada y de la lista de salida.
 */
class FilterMapProdReduce extends AbstractFilterMapReduce<number, number> {
  filter(pred: (item: number) => boolean, list: number[]): number[] {
    const result: number[] = [];
    for (const item of list) {
      if (pred(item)) {
 ...
 * @tparam number Tipo de los elementos de la lista de entrada y de la lista de salida.
 */
class FilterMapDivReduce extends AbstractFilterMapReduce<number, number> {
  filter(pred: (item: number) => boolean, list: number[]): number[] {
    const result: number[] = [];
    for (const item of list) {
      if (pred(item)) {
